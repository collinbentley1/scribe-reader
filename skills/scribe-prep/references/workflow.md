...
Read the private integration file identified by the event or requested workspace. Its `workspace`, `state_root`, `skill`, and `cloud_reader` fields supply all paths. Preserve the existing ledger when installing this skill. Read [state formats](state-format.md) before changing observations or preparation records.
...
1. Run `bun --no-env-file dist/watch.js target --config /absolute/integration.json` in `cloud_reader.source_directory`. Keep the returned receipt, `receiptPath`, and `receiptSha256` together. They identify immutable input. Do not substitute a newer target midway through review.
...
Use the configured Python executable with ReportLab already installed. Do not install dependencies during polls or model wakes.

```sh
"$PYTHON" "$SKILL/scripts/scribe_state.py" --root "$STATE_ROOT" render
"$PYTHON" "$SKILL/scripts/render_companion.py" --cards "$STATE_ROOT/output/cards.json" --output "$STATE_ROOT/output/companion.pdf"
```
